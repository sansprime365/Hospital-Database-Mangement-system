...
from STAFFHOMEPAGE import Staffmainpage
import tkinter
...
class LoginStaff:
    def __init__(self,root):
        ...
        main_frame.place(x=200,y=130,width=500,height=350)
        ...
        label=Label(main_frame,text="Staff Login",font=("times new roman",30,"bold"),bg="black",fg="white")
        ...
        registerbtn1=Button(main_frame,text="Register Account",command=self.rgster_window,width=11,font=("times new roman",12,"bold"),bg="black",fg="white",activebackground="black",activeforeground="white",borderwidth=0)
    ...
    def rgster_window(self):
        ...
        self.app=staffloginID(self.new_window)
    ...
    def login(self):
        if self.txtuser.get()==""or self.txtpass.get()=="":
            ...
            messagebox.showinfo("Login Success","Welcome to staff Database!")
        else:
            conn=mysql.connector.connect(host="localhost",username="root",password="Sh@15062005",database="project_database")
            my_cursor=conn.cursor()
            my_cursor.execute("select * from staffuserid where username=%s and Confirmpass=%s",(
                                                                                                self.txtuser.get(),
                                                                                                self.txtpass.get() 
                                                                                        ))
            ...
            if row==None:
                ...
                if open_menu>0:
                    ...
                    self.app=Staffmainpage(self.new_tab)
                else:
                    if not open_menu:
                        return
            conn.commit()
...
#=======================resetpassword========================
    def reset_pass(self):
        if self.security_combo.get()=="Select Question":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="Sh@15062005",database="project_database")
            my_cursor=conn.cursor()
            qury=("select * from staffuserid where username=%s and securityq=%s and securityans=%s")
            ...
            if row==None:
                ...
                query=("update staffuserid set Confirmpass=%s where username=%s")
...
#====================forgotpassword=======================
    def forgot_passwordid(self):
        if self.txtuser.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="Sh@15062005",database="project_database")
            my_cursor=conn.cursor()
            query=("select * from staffuserid where username=%s")
...
class staffloginID:
    def __init__(self,root):
        ...
        label=Label(main_frame,text="Staff UserID Registration",font=("times new roman",30,"bold"),bg="black",fg="white")
        label.place(x=160,y=0,width=480,height=60)

        name1=Label(main_frame,text="Staff User Name",font=("times new roman",15,"bold"),bg="black",fg="white")
        name1.place(x=50,y=60,width=200,height=60)
    ...
    def registerdata(self):
        if self.var_username.get()=="" or self.var_securityq.get()=="" or self.var_securityans.get()=="":
            ...
            if row!=None:
                ...
                my_cursor.execute("insert into staffuserid values(%s,%s,%s,%s,%s)",(
            ...
            conn.commit()
            conn.close()
            messagebox.showinfo("Success","Registered staff data")
